...
update_population (unsigned long *population, Rule *rules, int nrules)
{
  ...
  for (int i = 0; i < nrules; i++)
    {
      newPopulation[get_rule_id (rules, nrules, (rules[i].result1))]
          += *(population + i);
      newPopulation[get_rule_id (rules, nrules, (rules[i].result2))]
          += *(population + i);
    }
  ...
  while ((ret = sscanf (saveptr, "%2s", code)))
    {
      ...
      saveptr++;
      if (saveptr >= template + strlen (template) - 1)
        break;
    }
  ...
  for (int i = 0; i < 26; i++)
    {
      ...
      if (population[i] < minimum && population[i] != 0)
