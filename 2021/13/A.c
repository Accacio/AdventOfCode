...
paper_add_value(Paper * paper,int col,int row)
{
    ...
    if(row>=paper->rows)
    {
        paper->map=realloc(paper->map,(row+1)*paper->cols*sizeof(int));
        memset(paper->map+paper->rows*paper->cols,0,(row+1-paper->rows)*paper->cols*sizeof(int));
        paper->rows=row+1;
    }
...
main(int argc, char *argv[])
{
    ...
    if(strcmp(dir,"x")==0) fold_paper_horz(&paper,atoi(size));
    else fold_paper_vert(&paper,atoi(size));
    free(line);
