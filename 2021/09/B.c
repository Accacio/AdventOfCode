...
long
flood_fill(int * map,int * mark,int cols,int rows,int xInit,int yInit,int number)
{
    if(xInit<0||xInit>cols||yInit<0||yInit>rows||*(map+xInit+yInit*(cols))==9||*(mark+xInit+yInit*(cols))!=-1)
        return 0;
...
main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < count; i++) {
        long value = flood_fill(&map[0][0],&mark[0][0],cols+2,rows+2,minX[i],minY[i],i);
