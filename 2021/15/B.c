...
{
  return (*(map->data + (i % map->cols) + (j % map->rows) * map->cols)
          + i / map->cols + j / map->rows - 1)
             % 9
         + 1;
}
...
find_path (Map *map)
{
  ...
  *(map->costs) = 0;
      ...
      for (int i = 0; i < 4; i++)
        {
          ...
          if (neigi >= 0 && neigj >= 0 && neigi < map->costcols
              && neigj < map->costrows)
            {
              long *dist = (map->costs + neigi + neigj * map->costcols);
              long alt = *(map->costs + ci + cj * map->costcols)
                         + get_big_map (map, neigi, neigj);
              if (alt < *dist)
                {
                  *dist = alt;
  ...
  return *(map->costs + map->costcols - 1
           + (map->costrows - 1) * map->costcols);
}
